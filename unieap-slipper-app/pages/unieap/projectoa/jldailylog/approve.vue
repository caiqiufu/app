	...
	<view class="container">
		<view class="example">
			...
			<uni-forms ref="baseForm" :model="baseFormData" :rules="rules" labelWidth="80px">
				...
				<uni-forms-item label="审核结果">
					<radio-group @change="approveResultChange" class="labelbox">
						<label class="radio">
							<radio value="Pass" :checked="approveResult=='Pass'" />通过
						</label>
						<label class="radio" style="margin-left: 20px;">
							<radio value="Reject" :checked="approveResult=='Reject'" />驳回
						</label>
					</radio-group>					
				</uni-forms-item>
				<uni-forms-item label="审核意见">
					<uni-easyinput v-model="approveComment" placeholder="请输入审核意见" />
				</uni-forms-item>
				<view class="button-group">
					<button type="primary" size="mini" @click="submit()">确认</button>
				</view>
			</uni-forms>
		</view>
	</view>
...
<script>
	export default {
		...
		methods: {
			...
			submit() {
				uni.request({
					url: this.$baseUrl+'/backstage/projectoa/inf/approve?token=' +uni.getStorageSync("token"),
					method: 'POST',
					data: {
						logId:this.logId,
						projectId:this.baseFormData.projectId,
