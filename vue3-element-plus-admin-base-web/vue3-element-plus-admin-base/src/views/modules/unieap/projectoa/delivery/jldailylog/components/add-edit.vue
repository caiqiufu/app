    ...
    <el-form
      ...
      <el-row>
        <el-form-item
          ...
          <el-select
            ...
            @change="parentSelectedItemHandle(form.parentId)">
            <el-option
              v-for="item in optionsParentProjectList"
              :key="item.dicCode"
              :label="item.dicName"
              :value="item.dicCode" />
          </el-select>
        </el-form-item>
        <el-form-item label-width="80px" label="标段" prop="projectId">
          <el-select
            :disabled="isEdit"
            style="width: 150px"
      ...
      <el-row v-if="!isView" type="flex" justify="center">
        <el-form-item label-width="0px">
          ...
          <el-button v-repeat
                     type="primary"
                     @click="submit('Draft')">保存草稿</el-button>
          <el-button v-repeat
                     type="primary"
                     @click="submit('Pending')">提交审核</el-button>
        </el-form-item>
      </el-row>
    </el-form>
    <el-form
      ...
      <el-row>
        ...
        <el-form-item label="审核结果" prop="approveResult">
          <el-radio-group
            v-model="formApprove.approveResult"
  ...
  setup(_props, { emit }) {
    ...
    const initForm = (optType, id) => {
      ...
      projectDicDataApi({}).then((r) => {
        if (r) {
          data.optionsParentProjectList = r.data
        }
      })
      if (optType === 'Add') {
        data.isView = false
    ...
    const getProjectInfo = () => {
      ...
      bizHandleApi(params).then((r1) => {
        if (r1) {
          data.form.parentId = r1.data.parentId.toString()
          projectDicDataApi({ parentId: data.form.parentId }).then((r) => {
            if (r) {
              data.optionsProjectList = r.data
              data.form.projectId = data.projectId
            }
          })
        }
      })
    ...
    const parentSelectedItemHandle = (projectId) => {
      projectDicDataApi({ parentId: projectId }).then((r) => {
        if (r) {
          data.optionsProjectList = r.data
        }
      })
    ...
    const removeBuildingPart = (item) => {
      const index = data.form.deliveryBuildingpartList.indexOf(item)
      if (index !== -1) {
