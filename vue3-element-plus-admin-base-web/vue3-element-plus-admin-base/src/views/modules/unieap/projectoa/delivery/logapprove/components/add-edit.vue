<template>
  <el-dialog width="1000px" :title="form.id ? $t('unieap.comm.update') : $t('unieap.comm.create')" v-model="visible"
    :close-on-click-modal="false" @closed="dialogClosedHandle" append-to-body draggable destroy-on-close>
    <el-form v-loading="loading" :disabled="isView" :model="form" :rules="rules" ref="refForm" label-width="120px"
      label-position="right">
      <el-row>
        <el-form-item label-width="80px" :label="$t('unieap.projectoa.comm.projectName')" prop="projectName">
          <el-select :disabled="isEdit" style="width: 150px" v-model="form.projectId"
            :placeholder="$t('unieap.comm.plsSelect')" @change="selectedItemHandle(form.projectId)">
            <el-option v-for="item in optionsProjectList" :key="item.dicCode" :label="item.dicName" :value="item.dicCode">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label-width="80px" :label="$t('unieap.projectoa.comm.sgdw')" prop="sgdw">
          <el-input disabled style="width: 150px" v-model="form.sgdw" />
        </el-form-item>
        <el-form-item label-width="80px" label="日期" prop="logTime">
          <el-date-picker :disabled="isEdit" style="width: 150px" v-model="form.logTime" type="date"
      ...
      <el-row>
        <el-col :span="7">
          <el-form-item v-for="(buildingPartInfo, index) in form.deliveryBuildingpartList" label-width="80px"
            :label="'施工部位' + (index + 1)" :key="'sgbw_' + buildingPartInfo.code">
            <el-select filterable allow-create v-model="buildingPartInfo.sgbw" style="width: 200px"
              :placeholder="$t('unieap.comm.plsSelect')">
              <el-option v-for="item in optionsBuildingPartList" :key="item.dicCode" :label="item.dicName"
                :value="item.dicCode">
              </el-option>
            </el-select>
          </el-form-item>
        ...
        <el-col :span="7">
          <el-form-item v-for="(buildingPartInfo, index) in form.deliveryBuildingpartList" label-width="80px"
            :label="'施工内容' + (index + 1)" :key="'sglr_' + buildingPartInfo.code">
            <el-input type="textarea" :rows="1" v-model="buildingPartInfo.sglr" style="width: 230px" />
          </el-form-item>
        </el-col>
        <el-col :span="7">
          <el-form-item v-for="(buildingPartInfo, index) in form.deliveryBuildingpartList" label-width="80px"
            :label="'施工情况' + (index + 1)" :key="'sgqk_' + buildingPartInfo.code">
            <el-input type="textarea" :rows="1" v-model="buildingPartInfo.sgqk" style="width: 230px" />
          </el-form-item>
        </el-col>
        <el-col :span="3">
          <el-form-item v-for="(buildingPartInfo, index) in form.deliveryBuildingpartList" label-width="0px" :key="index">
            <el-button @click.prevent="removeBuildingPart(buildingPartInfo)"
              :disabled="form.deliveryBuildingpartList.length <= 1">{{ $t("unieap.comm.delete") }}</el-button>
          </el-form-item>
        </el-col>
        <el-form-item>
        ...
        <el-col :span="12">
          <el-row>
            <el-form-item label="巡检记录" prop="xjjl">
              <el-input type="textarea" :rows="3" v-model="form.xjjl" style="width: 280px" />
            </el-form-item>
          </el-row>
          <el-row>
        ...
        <el-col :span="12">
          <el-row>
            <el-form-item label="施工安全问题及处理后落实情况" prop="aqls">
              <el-input type="textarea" :rows="2" v-model="form.aqls" style="width: 280px" />
            </el-form-item>
          </el-row>
          <el-row>
            <el-form-item label="监理书面指令、执行及回复情况" prop="jlzlzx">
              <el-input type="textarea" :rows="2" v-model="form.jlzlzx" style="width: 280px" />
            </el-form-item>
          </el-row>
          <el-row>
            <el-form-item label="建设单位口头指示及处理意见" prop="ktzscl">
              <el-input type="textarea" :rows="2" v-model="form.ktzscl" style="width: 280px" />
            </el-form-item>
          </el-row>
          <el-row>
            <el-form-item label="施工单位汇报事项及处理意见" prop="hbsxcl">
              <el-input type="textarea" :rows="2" v-model="form.hbsxcl" style="width: 280px" />
            </el-form-item>
          </el-row>
          <el-row>
            <el-form-item label="其他事项" prop="qtsx">
              <el-input type="textarea" :rows="2" v-model="form.qtsx" style="width: 280px" />
            </el-form-item>
          </el-row>
        </el-col>
      </el-row>
      <el-row v-if="!isView" type="flex" justify="center">
        <el-form-item label-width="0px">
          ...
          <el-button v-repeat type="primary" @click="submit('Draft')">保存草稿</el-button>
          <el-button v-repeat type="primary" @click="submit('Pending')">提交审核</el-button>
        </el-form-item>
      </el-row>
    </el-form>
    <el-form v-loading="loading" v-if="isApprove" :model="formApprove" :rules="rules" ref="refFormApprove"
      ...
      <el-row>
        ...
        <el-form-item label="审核结果" prop="approveResult">
          <el-radio-group v-model="formApprove.approveResult" @change="radioChangeHandler">
            <el-radio v-for="approveResult in optionLogStatusList" :key="approveResult.dicCode"
              :label="approveResult.dicCode">{{ approveResult.dicName }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label-width="180px" label="总监理工程师意见" prop="approveComment">
          <el-input type="textarea" :rows="2" v-model="formApprove.approveComment" style="width: 700px" />
        </el-form-item>
      </el-row>
