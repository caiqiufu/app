<template>
  <el-dialog
    ...
    <el-form
      ...
      <el-form-item
        v-else-if="form.uiType === 'input'"
        :label="form.displayName"
        prop="attributeValue">
        <el-input
          v-model="form.attributeValue"
          :placeholder="$t('unieap.comm.plsInput')"
          style="width: 80%" /><span>({{ form.unit }})</span>
      </el-form-item>
      <el-form-item
        v-else-if="form.uiType === 'textarea'"
        :label="form.displayName"
        prop="attributeValue">
        <el-input
          v-model="form.attributeValue"
      ...
      <el-form-item
        v-else-if="form.uiType === 'date-picker'"
        :label="form.displayName"
        prop="attributeValue">
        <el-date-picker
          v-model="form.attributeValue"
    ...
    const rules = computed(() => {
      return {
        attributeValue: [
          {
            required: true,
            message: [t('unieap.comm.plsInput')] + [data.form.displayName],
            trigger: 'blur'
          }
        ]
      }
