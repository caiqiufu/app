<template>
  <el-dialog
    ...
    <el-form
      ...
      <el-form-item
        :label="$t('unieap.projectoa.projectmgt.projectinfo.parentProject')"
        prop="parentId">
        <el-select
          v-model="form.parentId"
          :placeholder="$t('unieap.comm.plsSelect')"
          style="width: 300px"
          @change="selectedItemHandle(form.parentId)">
          <el-option
      ...
      <el-form-item
        :label="$t('unieap.projectoa.comm.projectType')"
        prop="type">
        <el-select
          v-model="form.type"
          :placeholder="$t('unieap.comm.plsSelect')"
      ...
      <el-form-item
        :label="$t('unieap.projectoa.comm.bidAmount')"
        prop="bidAmount">
        <el-input
          v-model="form.bidAmount"
          :placeholder="$t('unieap.comm.plsInput')"
          maxlength="32"
          @input="bidAmountChanged">
      ...
      <el-form-item
        :label="$t('unieap.projectoa.comm.planstartDate')"
        prop="planstartDate">
        <el-date-picker
      ...
      <el-form-item
        :label="$t('unieap.projectoa.comm.planfinishDate')"
        prop="planfinishDate">
        <el-date-picker
      ...
      <el-form-item
        :label="$t('unieap.projectoa.comm.duration')"
        prop="duration">
        <el-input
  ...
  setup(_props, { emit }) {
        ...
        name: [
          {
            ...
            message:
              ...
              + [t('unieap.projectoa.comm.projectName')],
            trigger: 'blur'
          }
        ],
        type: [
          {
            ...
            message:
              [t('unieap.comm.plsInput')]
              + [t('unieap.projectoa.comm.projectType')],
            trigger: 'blur'
          }
        ],
        bidAmount: [
          {
            ...
            message:
              [t('unieap.comm.plsInput')]
              + [t('unieap.projectoa.comm.bidAmount')],
            validator: isNumber,
            trigger: 'blur'
          }
        ],
        planstartDate: [
          {
            ...
            message:
              [t('unieap.comm.plsInput')]
              + [t('unieap.projectoa.comm.planstartDate')],
            trigger: 'blur'
          }
        ],
        planfinishDate: [
          {
            ...
            message:
              [t('unieap.comm.plsInput')]
              + [t('unieap.projectoa.comm.planfinishDate')],
            trigger: 'blur'
          }
        ],
        duration: [
    ...
    const fieldValidate = () => {
      if (data.form.planfinishDate !== '' && data.form.planstartDate !== '') {
        const endTime = new Date(data.form.planfinishDate).getTime()
        const startTime = new Date(data.form.planstartDate).getTime()
    ...
    const setDuration = () => {
      if (data.form.planfinishDate !== '' && data.form.planstartDate !== '') {
        ...
        const startTime = new Date(data.form.planstartDate).getTime()
        if (endTime > startTime) {
          const dates = Math.ceil((endTime - startTime) / (1000 * 60 * 60 * 24))
