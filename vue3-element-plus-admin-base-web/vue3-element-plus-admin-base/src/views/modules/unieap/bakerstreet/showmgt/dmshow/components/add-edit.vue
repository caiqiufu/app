  ...
  <el-dialog width="600px" :title="form.id ? $t('unieap.comm.update') : $t('unieap.comm.create')" v-model="visible"
    ...
    <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="150px" label-position="right">
      ...
      <el-form-item :label="$t('unieap.bakerstreet.scriptmgt.dmName')" prop="playerId">
        <el-select v-model="form.playerId" :placeholder="$t('unieap.comm.plsSelect')" style="width:300px">
          <el-option v-for="item in optionsDms" :key="item.dicCode" :label="item.dicName" :value="item.dicCode">
          </el-option>
        </el-select>
      </el-form-item>
...
import { playerDicDataApi } from '@/api/unieap/bakerstreet/dic'
//自定包引入
import { updateDmShowApi } from '@/api/unieap/bakerstreet/playmgt'
...
export default defineComponent({
  ...
  setup(_props, { emit }) {
    ...
    const rules = computed(() => {
      return {
        playerId: [{ required: true, message: [t('unieap.comm.plsInput')] + [t('unieap.bakerstreet.scriptmgt.dmName')], trigger: 'blur' }],
     ...
     * @param {*} id 
     */
    const init = async (index, dmshow) => {
      const dicDms = await playerDicDataApi({playerType:'2'})
      if (dicDms) {
        data.optionsDms = dicDms.data
      }
      data.visible = true
      data.loading = true
      data.form.id = dmshow.id
      data.form.seq = index
      data.form.brief = dmshow.brief
      data.form.playerId = dmshow.playerId.toString()
      ...
      refForm.value.validate(async valid => {
        if (valid) {
          const params = {
            id: data.form.id,
            seq: data.form.seq,
            playerId: data.form.playerId,
            brief: data.form.brief
          }
          const r = await updateDmShowApi(params)
          if (r) {
            data.visible = false
            ElMessage({
