    ...
    <template #default>
      <el-table ref="refTable" v-loading="loading" :data="list" @selection-change="selectionHandle"
        ...
        <el-table-column align="center" :label="$t('unieap.bakerstreet.scriptmgt.scriptName')" prop="scriptName"
          width="200" />
        <el-table-column align="center" :label="$t('unieap.bakerstreet.playmgt.likeCount')" prop="likeCount"
          width="230" />
      </el-table>
    </template>
  </Container>
...
//自定包引入
import { scriptlikelistPageApi} from '@/api/unieap/bakerstreet/playmgt'
...
export default defineComponent({
  ...
  setup() {
    ...
    const getList = () => {
      data.loading = true
      const params = {}
      scriptlikelistPageApi(params).then(r => {
        if (r) {
