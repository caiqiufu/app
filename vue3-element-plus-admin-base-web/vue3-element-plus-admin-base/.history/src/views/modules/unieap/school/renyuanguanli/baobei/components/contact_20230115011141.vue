<template>
  <el-dialog width="900px" :title="form.id ? $t('unieap.comm.update') : $t('unieap.comm.create')" v-model="visible"
            ...
            <el-row :gutter="20" type="flex" justify="let">
              <el-col :span="12" align="left">
                <span>
                  ...
                  {{ contactInfo.priority === '1' ?'('+$t('unieap.school.renyuanguanli.baobei.isFirstContact') + ')' : ''}}
                </span>
              </el-col>
              <el-col :span="12" align="right">
                <el-button v-repeat v-show="contactInfo.priority !== '1'" type="primary" link
                  @click="deleteContactHandle(contactInfo.id)">{{ $t('unieap.comm.delete') }}</el-button>
                <el-button v-repeat type="primary" link @click="editContactHandle(contactInfo.id)">
                  {{ $t('unieap.comm.update') }}</el-button>
                <el-button v-repeat v-show="contactInfo.priority !== '1'" type="primary" link @click="setFirstContactHandle(contactInfo.id)">
                {{$t('unieap.school.renyuanguanli.baobei.setFirstContact')}}</el-button>
                <el-button v-repeat type="primary" link @click="resetPasswordtHandle(contactInfo.id)">
                  {{ $t('unieap.comm.resetPassword')}}</el-button>
              </el-col>
            </el-row>
          </template>
          <div class="text item">
            <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="80px"
              ...
              <el-row :gutter="10" type="flex" justify="start">
                <el-col :span="6">
                  ...
                  <el-form-item :label="$t('unieap.school.renyuanguanli.baobei.contactSourceType')"
                    prop="contactSourceType">
                    {{ contactInfo.sourceTypeDesc }}
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item :label="$t('unieap.school.renyuanguanli.baobei.relationship')" prop="nationality">
                ...
                <el-col :span="6">
                  <el-form-item :label="$t('unieap.school.renyuanguanli.baobei.contactNumber')" prop="nationality">
  ...
  <EditContact ref="refEditContact" v-if="editContactVisible" @refresh="getList" />
</template>
...
import { addKidsApi, contactInfoListApi, deleteContactInfoApi,setFirstContactInfoApi,resetPasswordContactInfoApi } from '@/api/unieap/school/renyuanguanli/baobei'
...
export default defineComponent({
   ...
   */
    ...
    const editContactHandle = (contactId) => {
      ...
      nextTick(() => {
        refEditContact.value.init({ 'kidsId': data.kidsId, 'contactId': contactId })
      })
    }
    /**
     * 删除联系人
    ...
    const deleteContactHandle = (contactId) => {
      ElMessageBox.confirm('<strong>删除须知：</strong></br>1.家长的所有信息都将清空</br>2.家长无法登录掌心宝贝APP</br>3.家长关联的所有付费服务都将无法恢复</br>', '提示', {
    ...
    const setFirstContactHandle = (contactId)=>{
      setFirstContactInfoApi({ 'kidsId': data.kidsId, 'contactId': contactId }).then(r => {
          if (r) {
            getList()
            ElMessage.success({
              message: t('unieap.comm.optSuccess'),
    ...
    const resetPasswordtHandle = (contactId)=>{
        ...
        ElMessageBox.confirm('确认重置当前账号密码？重置后，密码为APP默认登录密码。可在【园区设置>系统参数设置】中查看"APP默认登录密码"', '提示', {
        dangerouslyUseHTMLString: true,
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
