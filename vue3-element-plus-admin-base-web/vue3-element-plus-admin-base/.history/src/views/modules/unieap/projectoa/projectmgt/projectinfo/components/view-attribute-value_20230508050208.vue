<template>
  <el-dialog width="800px" :title="form.id ? $t('unieap.comm.update') : $t('unieap.comm.create')" v-model="visible"
    ...
    <template #default>
      <el-table ref="refTable" :data="dataList" v-loading="loading" :tree-props="props" row-key="attributeId" border>
        ...
        <el-table-column align="left" :label="$t('unieap.projectoa.attributemgt.attributeName')" prop="displayName"
          min-width="300" />
        <el-table-column align="left" :label="$t('unieap.projectoa.attributemgt.attributeValue')"
          prop="attributeValueDesc" min-width="200">
          <template v-slot="{ row }">
            <el-input
              v-if="row.displayName != null && row.displayName != '' && row.assigned === 'Y' && row.uiType === 'input'"
              v-model="row.attributeValue" disabled />
            <el-input
              v-if="row.displayName != null && row.displayName != '' && row.assigned === 'Y' && row.uiType === 'textarea'"
              type="textarea" :rows="3" v-model="row.attributeValue" disabled />
            <el-select disabled
              v-if="row.displayName != null && row.displayName != '' && row.assigned === 'Y' && row.uiType === 'select'"
              v-model="row.attributeValue">
              <el-option v-for="item in row.dicDataList" :key="item.dicCode" :label="item.dicName" :value="item.dicCode">
              </el-option>
            </el-select>
            <el-date-picker disabled
              v-if="row.displayName != null && row.displayName != '' && row.assigned === 'Y' && row.uiType === 'textarea'"
              type="date" v-model="row.attributeValue" />
          </template>
        </el-table-column>
...
import { assignAttributeApi, updateSeqApi, updateAttributeValueApi } from '@/api/unieap/projectoa/projectmgt/projectinfo'
