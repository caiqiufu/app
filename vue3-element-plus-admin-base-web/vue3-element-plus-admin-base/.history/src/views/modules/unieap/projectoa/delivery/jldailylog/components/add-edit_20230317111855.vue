<template>
  <el-dialog width="1000px" :title="form.id ? $t('unieap.comm.update') : $t('unieap.comm.create')" v-model="visible"
    ...
    <el-form v-loading="loading" :disabled="isView" :model="form" :rules="rules" ref="refForm" label-width="120px" label-position="right">
      <el-row>
        <el-form-item label-width="80px" :label="$t('unieap.projectoa.comm.projectName')" prop="projectName">
          <el-select :disabled="disEdit" style="width:150px" v-model="form.projectId"
        ...
        <el-form-item label-width="80px" label="日期" prop="logTime">
          <el-date-picker :disabled="disEdit" style="width:150px" v-model="form.logTime" type="date"
...
//自定包引入
import { projectDicDataApi, buildingPartDicDataApi } from '@/api/unieap/projectoa/dic'
...
export default defineComponent({
  ...
  setup(_props, { emit }) {
    ...
    const init = async (id) => {
      ...
      buildingPartDicDataApi({ projectId: data.projectId }).then(r => {
        if (r) {
        ...
        bizHandleApi(params).then(r => {
          if (r) {
            ...
            if (r.data.deliveryBuildingpartList.length === 0) {
              data.form.deliveryBuildingpartList = [{ id: Date.now(), projectId: data.form.projectId, logId: data.form.id, sgbw: '', sglr: '', sgqk: '' }]
            } else {
              data.form.deliveryBuildingpartList.forEach(function (element, index, array) {
                element.id = element.id.toString()
              })
            }
            buildingPartDicDataApi({ projectId: data.projectId }).then(r1 => {
              if (r1) {
    ...
    const addBuildingPart = () => {
      data.form.deliveryBuildingpartList.push({ id: Date.now(), projectId: data.projectId, logId: data.form.id, sgbw: '', sglr: '', sgqk: '' });
    }
