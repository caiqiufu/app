    ...
    <template #default>
      ...
      >
            ...
            <el-row :gutter="10" type="flex" justify="start">
              <el-col :span="18">
                <el-cascader
                  class="width-full"
                  ref="refCascader"
                  v-model="row.employeeIds"
                  :options="optionEmployeeList"
                  :props="cascaderProps"
              ...
              <el-col :span="6">
                <el-button @click.prevent="updateTaskInfoHandle(row)">{{
                  $t("unieap.comm.save")
                }}</el-button>
              </el-col>
            </el-row>
        ...
        >
          <template v-slot="{ row }">
            <el-switch
              @change="updateTaskInfoHandle(row)"
...
//自定包引入
import { updateTaskInfoApi } from "@/api/unieap/projectoa/projectmgt/projectinfo";
...
export default defineComponent({
  ...
  setup(_props, { emit }) {
    ...
    const init = async (projectId) => {
      ...
      bizHandleApi({
        url: urlPrefix + "allEmployeeDicDataList",
      }).then((r) => {
        if (r) {
    ...
    //修改Task信息
    const updateTaskInfoHandle = (row) => {
      if (row.taskId) {
        updateTaskInfoApi({
          id: row.id,
          projectId: data.projectId,
          employeeId: row.employeeIds?row.employeeIds.join(','):'',
          taskId: row.taskId,
          notifyFlag: row.notifyFlag,
        }).then((r) => {
          if (r) {
            ElMessage({
              message: "操作成功!",
