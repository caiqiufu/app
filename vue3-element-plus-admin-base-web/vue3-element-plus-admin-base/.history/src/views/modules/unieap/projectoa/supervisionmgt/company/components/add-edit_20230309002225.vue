  ...
  <el-dialog width="600px" :title="form.id ? $t('unieap.comm.update') : $t('unieap.comm.create')" v-model="visible"
    ...
    <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="150px" label-position="right">
      ...
      <el-form-item :label="$t('unieap.projectoa.supervisionmgt.company.businessCorporation')" prop="businessCorporation">
        <el-input v-model="form.businessCorporation" :placeholder="$t('unieap.comm.plsInput')" maxlength="32" style="width:300px"/>
      </el-form-item>
...
export default defineComponent({
  ...
  setup(_props, { emit }) {
    ...
    const refForm = ref()
    //数据定义
    const urlPrefix = '/backstage/projectoa/supervisionmgt/company/'
    const data = reactive({
      loading: false,
      visible: false,
      optionsCompanyType: [],
    ...
    const rules = computed(() => {
      return {
        ...
        corporation: [{ required: true, message: [t('unieap.comm.plsInput')] + [t('unieap.projectoa.supervisionmgt.company.corporation')], trigger: 'blur' }]
      }
    })
    /**
     * @description: 加载窗口初始化
