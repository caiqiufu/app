    ...
    <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="120px" label-position="right">
      <el-row>
        <el-form-item :label="$t('unieap.projectoa.comm.projectName')" prop="projectName">
          <el-select v-if="form.id === null" v-model="form.projectId" :placeholder="$t('unieap.comm.plsSelect')" @change="selectedItemHandle(form.projectId)">
            <el-option v-for="item in optionsProjectList" :key="item.dicCode" :label="item.dicName" :value="item.dicCode">
            </el-option>
          </el-select>
          <el-input v-else v-model="form.projectName" style="width:200px" disabled />
        </el-form-item>
        <el-form-item label="日期" prop="logTime">
          <el-date-picker v-if="form.id === null" v-model="form.logTime" type="date"
            :placeholder="$t('unieap.comm.plsSelect')">
          </el-date-picker>
          <el-input v-else v-model="form.logTime" style="width:200px" disabled />
        </el-form-item>
      </el-row>
      ...
      <el-row>
        <el-form-item label="施工部位" prop="sgbw">
          <el-input type="textarea" :rows="1" v-model="form.sgbw" style="width:600px" />
        </el-form-item>
      </el-row>
      <el-row>
        <el-form-item label="施工内容" prop="sglr">
          <el-input type="textarea" :rows="1" v-model="form.sglr" style="width:600px" />
        </el-form-item>
      </el-row>
      <el-row>
        <el-form-item label="施工情况" prop="sgqk">
          <el-input type="textarea" :rows="1" v-model="form.sgqk" style="width:600px" />
        </el-form-item>
      </el-row>
      ...
      <el-row>
        <el-form-item label="施工安全问题及处理后落实情况" prop="aqls">
          <el-input type="textarea" :rows="1" v-model="form.aqls" style="width:600px" />
        </el-form-item>
      </el-row>
      <el-row>
        <el-form-item label="监理书面指令、执行及回复情况" prop="jlzlzx">
          <el-input type="textarea" :rows="1" v-model="form.jlzlzx" style="width:600px" />
        </el-form-item>
      </el-row>
      <el-row>
        <el-form-item label="建设单位口头指示及处理意见" prop="ktzscl">
          <el-input type="textarea" :rows="1" v-model="form.ktzscl" style="width:600px" />
        </el-form-item>
      </el-row>
      <el-row>
        <el-form-item label="施工单位汇报事项及处理意见" prop="hbsxcl">
          <el-input type="textarea" :rows="1" v-model="form.hbsxcl" style="width:600px" />
        </el-form-item>
      </el-row>
      <el-row>
