    ...
    <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="150px" label-position="right">
      ...
      <el-form-item :label="$t('unieap.projectoa.supervisionmgt.employee.certificateType')" prop="certificateTypeDesc">
        <el-select v-model="form.certificateType" :placeholder="$t('unieap.comm.plsSelect')" style="width:300px">
          <el-option v-for="item in optionsCertificateType" :key="item.dicCode" :label="item.dicName" :value="item.dicCode">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item :label="$t('unieap.projectoa.supervisionmgt.employee.certificateLicense')" prop="certificateLicense">
        <el-input v-model="form.certificateLicense" :placeholder="$t('unieap.comm.plsInput')" maxlength="32" show-word-limit
          style="width:300px" />
      </el-form-item>
...
export default defineComponent({
  ...
  setup(_props, { emit }) {
     ...
     */
    const init = async (id) => {
      const dicPositionType = await infoDicDataApi({ 'groupCode': 'EMPLOYEE_POSITION' })
      if (dicPositionType) {
        data.optionsPositionType = dicPositionType.data
      }
      const dicCertificateType = await infoDicDataApi({ 'groupCode': 'CERTIFICATE_TYPE' })
      if (dicCertificateType) {
        data.optionsCertificateType = dicCertificateType.data
      }
      data.visible = true
      data.loading = true
      data.form.id = id
