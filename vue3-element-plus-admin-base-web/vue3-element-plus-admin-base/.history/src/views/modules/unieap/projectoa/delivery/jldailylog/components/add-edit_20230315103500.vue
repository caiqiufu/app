        ...
        <el-col :span="3">
          <el-form-item v-for="(buildingPartInfo, index) in form.deliveryBuildingpartList" label-width="0px" :key="index">
            <el-button @click.prevent="removeBuildingPart(buildingPartInfo)"
              :disabled="(form.buildingPartList.length <= 1)">{{
  ...
  setup(_props, { emit }) {
    ...
    const data = reactive({
      ...
      form: {
        ...
        modifyDate: '',
        modifyBy: '',
        deliveryBuildingpartList: [{ id: Date.now(), projectId: '', sgbw: '', sglr: '', sgqk: '' }]
      },
      roles: []
    })
    ...
    const init = async (id) => {
      ...
      data.form.deliveryBuildingpartList = [{ id: Date.now(), projectId: '', sgbw: '', sglr: '', sgqk: '' }]
      data.visible = true
    }
    ...
    const addBuildingPart = () => {
      data.form.deliveryBuildingpartList.push({ id: Date.now(), projectId: '', sgbw: '', sglr: '', sgqk: '' });
    }
