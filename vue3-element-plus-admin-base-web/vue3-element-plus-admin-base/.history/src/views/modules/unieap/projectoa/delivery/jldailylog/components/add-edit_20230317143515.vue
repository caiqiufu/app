      ...
      <el-row>
        ...
        <el-form-item label="审核结果" prop="approveResult">
          <el-radio-group v-model="form.approveResult">
            <el-radio v-for="approveResult in optionLogStatusList" :key="approveResult.dicCode" :label="approveResult.dicCode">{{
              approveResult.dicName
            }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label-width="180px" label="总监理工程师意见" prop="aqls">
          <el-input
            type="textarea"
            :rows="2"
            v-model="form.aqls"
...
export default defineComponent({
  ...
  setup(_props, { emit }) {
    ...
    const initForm = (optType, id) => {
      ...
      data.optionLogStatusList = [{dicCode:'Pass',dicName:'审核通过'},{dicCode:'Reject',dicName:'驳回'}]
    ...
    const submit = (optType) => {
      refForm.value.validate(async (valid) => {
        if (valid) {
          ...
          data.form.status = optType==='Draft'?'Draft':'Pending'
