    ...
    <template #header>
      <el-form ref="refForm" :inline="true" @keyup.enter="reacquireHandle()">
        <el-form-item :label="$t('unieap.projectoa.supervisionmgt.company.companyName')" prop="companyName">
          <el-input v-model="form.companyName" :placeholder="$t('unieap.comm.plsInput')" />
        </el-form-item>
        <el-form-item :label="$t('unieap.projectoa.supervisionmgt.employee.name')" prop="employeeName">
          <el-input v-model="form.employeeName" :placeholder="$t('unieap.comm.plsInput')" />
        </el-form-item>
        <el-form-item>
          ...
          <el-button v-permission="'supervisionmgt.employee:create'" type="primary" @click="addEditHandle()">
            <el-icon>
              <Plus />
            </el-icon>
            {{
              $t('unieap.projectoa.supervisionmgt.employee.create')
            }}</el-button>
          <el-button v-permission="'supervisionmgt.employee:delete'" type="danger" :disabled="selection.length <= 0"
            @click="deleteHandle()">
            <el-icon>
              <Delete />
            </el-icon>
  ...
  setup() {
    ...
    const getList = () => {
      const params = {
        ...
        name: data.form.employeeName
      }
      data.loading = true
      params.url = urlPrefix + 'employeeInfoPage'
