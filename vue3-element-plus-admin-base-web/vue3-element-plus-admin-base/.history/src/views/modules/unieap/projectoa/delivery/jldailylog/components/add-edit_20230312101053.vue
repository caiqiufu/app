...
export default defineComponent({
  ...
  setup(_props, { emit }) {
    ...
    const data = reactive({
      loading: false,
      visible: false,
      optionsWeatherMorningType: [],
      optionsWeatherAfternoonType:[],
      form: {
        id: null,
        projectId: '',
        projectName: '',
     ...
     */
    const init = async (id) => {
      const dicWeatherType = await infoDicDataApi({ 'groupCode': 'WEATHER_TYPE' })
      if (dicWeatherType) {
        data.optionsWeatherMorningType = dicWeatherType.data
        data.optionsWeatherAfternoonType = dicWeatherType.data
      }
