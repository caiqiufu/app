  ...
  <el-dialog width="600px" :title="form.id ? $t('unieap.comm.update') : $t('unieap.comm.create')" v-model="visible"
    ...
    <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="150px" label-position="right">
      ...
      <el-form-item label="提醒周期" prop="notifyCron">
        <el-input type="textarea" :rows="3" v-model="form.notifyCron" :placeholder="$t('unieap.comm.plsInput')" style="width:300px"/>
      </el-form-item>
      <el-form-item label="提醒消息模版" prop="messageTemplate">
        <el-input type="textarea" :rows="3" v-model="form.messageTemplate" :placeholder="$t('unieap.comm.plsInput')" style="width:300px"/>
      </el-form-item>
...
export default defineComponent({
  ...
  setup(_props, { emit }) {
    ...
    const rules = computed(() => {
      return {
        name: [{ required: true, message: [t('unieap.comm.plsInput')] + '任务名称', trigger: 'blur' }],
        type: [{ required: true, message: [t('unieap.comm.plsInput')] + '任务类型', trigger: 'blur' }],
        notifyFlag: [{ required: true, message: [t('unieap.comm.plsInput')] + '是否提醒', trigger: 'blur' }],
        notifyCron: [{ required: true, message: [t('unieap.comm.plsInput')] + '提醒周期', trigger: 'blur' }]
      }
    })
    /**
     * @description: 加载窗口初始化
