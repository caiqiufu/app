      ...
      <el-scrollbar style="height: 100%">
        <el-table ref="refTable" :data="dataList" v-loading="loading" :tree-props="props" row-key="attributeId" border>
          ...
          <el-table-column align="left" :label="$t('unieap.projectoa.attributemgt.attributeName')" prop="displayName"
            min-width="300" />
          <el-table-column align="center" :label="$t('unieap.projectoa.attributemgt.attributeSeq')" prop="seq" width="80">
            <template v-slot="{ row }">
              <el-input v-if="row.displayName != null && row.displayName != '' && row.assigned === 'Y'"
                @blur="seqEditHandle(row)" v-model="row.seq" />
            </template>
          </el-table-column>
          <el-table-column align="center" :label="$t('unieap.projectoa.projectmgt.projectinfo.assignAttribute')"
            prop="assigned" width="200">
            <template v-slot="{ row }">
              <el-switch v-if="row.displayName != null && row.displayName != ''" @change="assignAttributeHandle(row)"
                v-model="row.assigned" active-value="Y" inactive-value="N" />
            </template>
          </el-table-column>
        </el-table>
      </el-scrollbar>
...
import { assignAttributeApi, updateSeqApi } from '@/api/unieap/projectoa/projectmgt/projectinfo'
  ...
  setup() {
    ...
    const getList = () => {
      if (data.projectId) {
        data.loading = true
        const params = {
          url: urlPrefix + 'allAttributeList',
          // 查询参数
