<template>
  <Container>
        ...
        <el-row :gutter="5" type="flex" justify="center">
          ...
          <el-col :span="9">
            ...
            <el-button
              ...
              :disabled="
                selection.length !== 1 ||
                (selection.length == 1 && selection[0].status !== 'Pending')
              "
              @click="approveHandle()"
            >
              {{
    ...
    <template #default>
        ...
        >
          <template v-slot="{ row }">
            ...
            <el-button
              ...
              type="danger"
              link
              @click="approveHandle(row)"
              >{{
                $t("unieap.projectoa.delivery.jldailylog.approve")
              }}</el-button
            >
            <el-button
              v-permission="'delivery.jldailylog:approveLog'"
              type="danger"
              link
              @click="approveLogHandle(row.id)"
              >{{
                $t("unieap.projectoa.delivery.jldailylog.approveLog")
              }}</el-button
            >
          </template>
        </el-table-column>
      ...
      <ViewApproveLog ref="refViewApproveLog" v-if="visibleViewApproveLog" />
    </template>
    <template #footer>
...
import ViewApproveLog from "./components/view-approvelog";
import { infoDicDataApi, bizHandleApi } from "@/api/unieap/comm";

export default defineComponent({
  components: { AddEdit, ViewApproveLog },
  setup() {
    const { t } = useI18n();
    ...
    const viewLogHandle = (row) => {
      ...
      if (row == null) {
        ...
        row = data.selection[0];
      }
      data.visible = true;
      nextTick(() => {
        if (row) {
          refAddEdit.value.initView(row.id);
        }
      });
    };
    //审核
    const approveHandle = (row) => {
      ...
      if (row.status === "Pending") {
        ...
        nextTick(() => {
          if (row) {
            refAddEdit.value.initApprove(row.id);
          }
        });
      } else {
        ElMessage({
    ...
    const approveLogHandle = (id) => {
      ...
      data.visibleViewApproveLog = true;
      nextTick(() => {
        if (id) {
          refViewApproveLog.value.init(id);
        }
      });
    };
    ...
    //查询
    const initLoadData = () => {
      if (router.query.logId) {
        data.logId = router.query.logId;
      }
    };
    onBeforeMount(() => {
      initLoadData();
