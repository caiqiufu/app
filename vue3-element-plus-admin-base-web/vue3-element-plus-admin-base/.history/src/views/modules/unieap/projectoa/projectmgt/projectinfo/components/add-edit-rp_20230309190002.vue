<template>
  <el-dialog width="600px" :title="$t('unieap.comm.edit')" v-model="visible"
    ...
    <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="150px" label-position="right">
      ...
      <el-form-item v-if="addVisible" :label="$t('unieap.projectoa.comm.projectName')" prop="projectId">
        <el-select  v-model="form.projectId" :placeholder="$t('unieap.comm.plsSelect')" style="width:300px">
          <el-option v-for="item in optionsProject" :key="item.dicCode" :label="item.dicName" :value="item.dicCode">
          </el-option>
        </el-select>      
      </el-form-item>
      <el-form-item  v-else :label="$t('unieap.projectoa.comm.projectName')" prop="projectName">
        <el-input v-model="form.projectName" :placeholder="$t('unieap.comm.plsInput')" maxlength="32"
