  ...
  <el-dialog width="1000px" :title="form.id ? $t('unieap.comm.update') : $t('unieap.comm.create')" v-model="visible"
    ...
    <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="120px" label-position="right">
      ...
      <el-divider content-position="left">
        <font style="font-weight: bold;">天气情况</font>
      </el-divider>
      <el-row>
        <el-col :span="6">
      ...
      <el-divider content-position="left">
        <font style="font-weight: bold;">施工情况</font>
      </el-divider>
      <el-row>
        <el-col :span="7">
          <el-form-item v-for="(buildingPartInfo, index) in form.buildingPartList" label-width="80px"
        ...
        <el-col :span="3">
          <el-form-item v-for="(buildingPartInfo, index) in form.buildingPartList" label-width="0px" :key="index">
            <el-button @click.prevent="removeBuildingPart(buildingPartInfo)" :disabled="(form.buildingPartList.length <= 1)">{{
      ...
      <el-divider content-position="left">
        <font style="font-weight: bold;">监理巡检记录</font>
      </el-divider>
      <el-row>
        <el-col :span="12">
          <el-row>
  ...
  setup(_props, { emit }) {
    ...
    const data = reactive({
      ...
      form: {
        ...
        modifyDate: '',
        modifyBy: '',
        buildingPartList: [{code: Date.now(), sgbw: '', sglr: '', sgqk: '' }]
      },
      roles: []
    })
    ...
    const init = async (id) => {
      ...
      data.form.buildingPartList = [{code: Date.now(), sgbw: '', sglr: '', sgqk: '' }]
      data.visible = true
    }
      ...
      if (id) {
        ...
        bizHandleApi(params).then(r => {
          if (r) {
            ...
            buildingPartDicDataApi({ projectId: data.form.projectId }).then(r1 => {
              if (r1) {
    ...
    const removeBuildingPart = (item) => {
      var index = data.form.buildingPartList.indexOf(item)
      if (index !== -1) {
        data.form.buildingPartList.splice(index, 1)
      }
    }

    const addBuildingPart = () => {
      data.form.buildingPartList.push({ code: Date.now(), name: '' });
    }
