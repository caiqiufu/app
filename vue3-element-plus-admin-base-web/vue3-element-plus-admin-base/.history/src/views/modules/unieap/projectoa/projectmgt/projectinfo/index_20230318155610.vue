    ...
    <template #header>
      <el-form ref="refForm" :inline="true" @keyup.enter="reacquireHandle()" label-width="80px">
        ...
        <el-row :gutter="5" type="flex" justify="center">
          ...
          <el-col :span="9">
            <el-button v-permission="'projectmgt.projectinfo:editAttributeDefine'" type="primary"
            :disabled="selection.length != 1" @click="addEditAttributeDefineHandle()">
              {{
                $t('unieap.projectoa.projectmgt.projectinfo.editAttributeDefine')
              }}</el-button>
            <el-button v-permission="'projectmgt.projectinfo:editAttributeDetail'" type="primary"
            :disabled="selection.length != 1" @click="addEditAttributeDetailHandle()">
              {{
                $t('unieap.projectoa.projectmgt.projectinfo.editAttributeDetail')
              }}</el-button>
            <el-button v-permission="'projectmgt.projectinfo:editRP'" type="primary" :disabled="selection.length != 1"
              @click="editRPHandle()">
              {{ $t('unieap.projectoa.projectmgt.projectinfo.editRP') }}</el-button>
              <el-button v-permission="'projectmgt.projectinfo:editTask'" type="primary" :disabled="selection.length != 1"
              @click="editTaskHandle()">编辑任务</el-button>
          </el-col>
        </el-row>
      </el-form>
    </template>
    <template #default>
      <el-table ref="refTable" v-loading="loading" :data="list" @selection-change="selectionHandle"
        ...
        <el-table-column align="center" :label="$t('unieap.projectoa.comm.projectName')" prop="name" width="200"
          sortable='custom' />
        <el-table-column align="center" :label="$t('unieap.projectoa.comm.bidAmount')" prop="bidAmount" width="200"
          :formatter="moneyFormatter" />
        <el-table-column align="center" :label="$t('unieap.projectoa.comm.bidDate')" prop="bidDate" width="150"
          :formatter="dateFormatter" />
        <el-table-column align="center" :label="$t('unieap.projectoa.comm.planstartDate')" prop="planstartDate"
          width="150" :formatter="dateFormatter" />
        <el-table-column align="center" :label="$t('unieap.projectoa.comm.planfinishDate')" prop="planfinishDate"
          width="150" :formatter="dateFormatter" />
      ...
      <ViewAttributeValue ref="refViewAttributeValue" v-if="visibleViewAttributeValue" @refresh="getList" />
      ...
      <AddEditRP ref="refAddEditRP" v-if="visibleAddEditRP" @refresh="getList" />
      <AddEditTask ref="refAddEditTask" v-if="visibleAddEditTask" @refresh="getList" />
    </template>
    <template #footer>
...
import ViewAttributeValue from './components/view-attribute-value'
import AddEditRP from './components/add-edit-rp'
import AddEditTask from './components/add-edit-task'
import { bizHandleApi } from '@/api/unieap/comm'

export default defineComponent({
  components: { AddEdit, AddEditAttributeDefine, AddEditAttributeValue, ViewAttributeValue, AddEditRP,AddEditTask },
  setup() {
    const { t } = useI18n()
    const refForm = ref()
    const refTable = ref()
    ...
    const refViewAttributeValue = ref()
    const refAddEditRP = ref()
    const refAddEditTask = ref()
    const { page } = usePage()
    //数据定义
    ...
    const data = reactive({
      ...
      visibleViewAttributeValue: false,
      visibleAddEditRP: false,
      visibleAddEditTask: false,
      form: {
        sort: 'name',
        dir: 'asc',
        projectName:'',
    ...
    const viewAttributeDefineHandle = () => {
      data.visibleViewAttributeValue = true
      ...
      nextTick(() => {
        refViewAttributeValue.value.init(projectId)
      })
    }
    //编辑
    ...
    const editRPHandle = () => {
      ...
      const projectId = data.selection[0].id
      nextTick(() => {
        refAddEditRP.value.init(projectId)
      })
    }

    //编辑项目任务
    const editTaskHandle = () => {
      data.visibleAddEditTask = true
      if (data.selection.length !== 1) {
        ElMessage({
          message: t('unieap.comm.selectOne', { dataType: '项目' }),
          type: 'warning'
      ...
      const projectId = data.selection[0].id
      nextTick(() => {
        refAddEditTask.value.init(projectId)
      })
    }
    //选中
