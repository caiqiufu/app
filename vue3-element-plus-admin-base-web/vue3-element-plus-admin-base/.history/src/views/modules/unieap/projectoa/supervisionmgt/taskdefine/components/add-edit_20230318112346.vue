<template>
  ...
  >
    ...
    >
      ...
      <el-form-item
        :label="$t('unieap.projectoa.supervisionmgt.company.businessLicense')"
      ...
      <el-form-item label="是否提醒" prop="notifyFlag">
        <el-select
          v-model="form.notifyFlag"
          :placeholder="$t('unieap.comm.plsSelect')"
          style="width: 300px"
        >
          <el-option
            v-for="item in optionNotifyFlag"
            :key="item.dicCode"
            :label="item.dicName"
            :value="item.dicCode"
          >
    ...
    const rules = computed(() => {
      return {
        ...
        notifyFlag: [
          {
            required: true,
            message: [t("unieap.comm.plsInput")] + "是否提醒",
            trigger: "blur",
          },
        ],
        notifyCron: [
