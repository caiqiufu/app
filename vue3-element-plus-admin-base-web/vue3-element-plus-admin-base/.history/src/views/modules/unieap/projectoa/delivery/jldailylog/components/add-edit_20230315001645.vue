<template>
  <el-dialog width="1000px" :title="form.id ? $t('unieap.comm.update') : $t('unieap.comm.create')" v-model="visible"
    ...
    <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="120px" label-position="right">
      ...
      <el-row>
        <el-col :span="8">
          <el-form-item v-for="(buildingPartInfo, index) in form.buildingPartList"
          :label="施工部位 + (index + 1)" :key="buildingPartInfo.sgbw">
          <el-input v-model="buildingPartInfo.sgbw" 
      ...
      <el-row v-for="(buildingPartInfo, index) in form.buildingPartList">
        <el-form-item label-width="80px" :label="施工部位 + (index + 1)" :key="index">
          <el-select filterable allow-create v-model="buildingPartInfo.sgbw" style="width:200px" :placeholder="$t('unieap.comm.plsSelect')">
