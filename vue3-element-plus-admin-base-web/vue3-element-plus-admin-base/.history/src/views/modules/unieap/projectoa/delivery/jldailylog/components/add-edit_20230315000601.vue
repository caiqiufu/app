  ...
  <el-dialog width="1000px" :title="form.id ? $t('unieap.comm.update') : $t('unieap.comm.create')" v-model="visible"
    ...
    <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="120px" label-position="right">
      ...
      <el-row v-for="(buildingPartInfo, index) in form.buildingPartList">
        <el-form-item label-width="80px" :label="施工部位 + (index + 1)" :key="buildingPartInfo.sgbw">
        ...
        <el-form-item label-width="80px" :label="施工内容 + (index + 1)" :key="buildingPartInfo.sglr">
          <el-input type="textarea" :rows="1" v-model="buildingPartInfo.sglr" style="width:230px" />
        </el-form-item>
        <el-form-item label-width="80px" :label="施工情况 + (index + 1)" :key="buildingPartInfo.sgqk">
