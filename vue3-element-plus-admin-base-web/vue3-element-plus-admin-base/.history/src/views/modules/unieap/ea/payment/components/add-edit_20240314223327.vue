  ...
  <el-dialog width="600px" :title="form.id ? $t('unieap.comm.update') : $t('unieap.comm.create')" v-model="visible"
    ...
    <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="150px" label-position="right">
      ...
      <el-form-item label="支付凭证" prop="url">
        <UImageUploadSingle v-model="form.file" @uploadFinishedHandle="uploadFinishedHandle" category="form.category"
...
import UImageUploadSingle from '@/components/unieap/ile-upload-single'
export default defineComponent({
  components: { UImageUploadMutiple ,UImageUploadSingle},
  emits: ["refresh"],
  setup(_props, { emit }) {
    const { t } = useI18n();
    const refForm = ref();
