  ...
  <Container>
    <template #header>
      <el-form v-loading="loading" :model="form" :rules="rules" ref="refForm" label-width="150px"
        ...
        <el-form-item style="text-align: center">
          <el-button type="primary" @click="handlePrev">上一步，填写角色信息</el-button>
          <el-button type="primary" @click="handleNext">下一步，填写线索信息</el-button>
          <el-button type="primary" @click="addEditHandle({id:'',scriptId:''})">
            <el-icon>
              <Plus />
            </el-icon>
            {{
              $t('unieap.bakerstreet.scriptmgt.createChapter')
            }}</el-button>
        </el-form-item>
      </el-form>
    </template>
    <template #default>
      <el-table ref="refTable" v-loading="loading" :data="list" @selection-change="selectionHandle"
        ...
        <el-table-column align="center" :label="$t('unieap.bakerstreet.scriptmgt.chapterNumber')" prop="chapterNumberDesc" width="100" />
        <el-table-column align="center" :label="$t('unieap.bakerstreet.scriptmgt.roleName')" prop="roleName" width="100" />
...
//自定包引入
import AddEdit from './add-edit-chapter'
import {chapterPageApi} from '@/api/unieap/bakerstreet/scriptmgt'
    ...
    const deleteHandle = (id) => {
      ...
      ElMessageBox.confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        const params = {
          ...
          keys: keys,
          operType: 'delete',
          repositoryName: 'chapterInfoRepository',
          beanName: 'bakerstreet.scriptmgt.pojo.ChapterInfo',
          boName: '',
        }
