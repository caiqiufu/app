...
const URL = 'ws://localhost:8816/slipper/websocket/263ba199f1863e9a2256cdc86f3679a4'
let timer = null
let ws = null
let isConnect = false
const checkHeart = 'check-heart'
let count = 0
...
function receive(data) {
  const event = new CustomEvent('receive', { detail: data })
  window.dispatchEvent(event)
}
function connectStatus(status) {
  const event = new CustomEvent('connectStatus', { detail: status })
  window.dispatchEvent(event)
}
