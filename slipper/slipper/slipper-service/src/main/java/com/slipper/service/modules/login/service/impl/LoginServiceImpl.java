...
public class LoginServiceImpl implements LoginService {
	...
	public TokenEntity backstageLogin(BackstageLoginForm backstageLoginForm) {
		boolean result = captchaService.validateCode(backstageLoginForm.getUuid(), backstageLoginForm.getCode(),
				Constant.STORAGE_MODE);
		if (!result) {
		...
		if (administratorEntity == null) {
			...
			message = "账户已被冻结！";
		} else if (!administratorEntity.getPassword()
				.equals(new Sha256Hash(backstageLoginForm.getPassword(), administratorEntity.getSalt()).toHex())) {
			message = "密码不正确！";
		}
		...
		// 添加登录日志
		addLog(backstageLoginForm, administratorEntity != null ? administratorEntity.getId() : null,
	...
	@Override
	public TokenEntity infAuth(String userName, String password) {
		String message = null;
		// 验证账户
		LambdaQueryWrapper<AdministratorEntity> wrapper = new LambdaQueryWrapper<AdministratorEntity>()
				.eq(AdministratorEntity::getUsername, userName);
		AdministratorEntity administratorEntity = administratorService.getOne(wrapper);
		if (administratorEntity == null) {
			message = "帐号不存在！";
		} else if (administratorEntity.getStatus() == 0) {
			message = "账户已被冻结！";
				...
				.equals(new Sha256Hash(password, administratorEntity.getSalt()).toHex())) {
			message = "密码不正确！";
		}
		// 验证企业
		if (StringUtils.isBlank(message)) {
		...
		BackstageLoginForm backstageLoginForm = new BackstageLoginForm();
		backstageLoginForm.setUsername(userName);
		backstageLoginForm.setPassword(password);
		// 添加登录日志
		addLog(backstageLoginForm, administratorEntity != null ? administratorEntity.getId() : null,
				StringUtils.isBlank(message) ? "登录成功" : message);
		if (StringUtils.isNotBlank(message)) {
		...
		TokenEntity tokenEntity = tokenService.selectById(administratorEntity.getId());
		if (tokenEntity == null || tokenEntity.getExpiredAt().getTime() < System.currentTimeMillis()) {
			tokenEntity = tokenService.createToken(administratorEntity.getId(), Constant.STORAGE_MODE);
		}
		return tokenEntity;
	}
