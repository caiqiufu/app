...
public class UnieapConstants {
	 ...
	 * 数据源
	 */
	public final static Map<String, JdbcTemplate> DS = new HashMap<String, JdbcTemplate>();

	/**
	 * 系统数据配置
	 */
	public final static Map<String, String> SYS_DATA = new HashMap<String, String>();
	 ...
	 * EA 策略平台
	 */
	public static String APP_CODE_EA = "EA";
	...
	public static DicDataVO getDicData(String groupCode, String dicCode) {
		...
		return baseBO.getDicData(language, groupCode, dicCode);
	}

	public static DicDataVO getDicData(String groupCode, String dicCode, String dicName) {
		if (StringUtils.isNotEmpty(dicCode)) {
			return getDicData(groupCode, dicCode);
		} else {
			BaseBO baseBO = (BaseBO) ApplicationContextProvider.getBean("baseBO");
			DicGroupVO dicGroupVO = baseBO.getDicGroup(groupCode);
			List<DicDataVO> dataList = dicGroupVO.getDataList();
			if (dataList != null && dataList.size() > 0) {
				for (DicDataVO vo : dataList) {
					if (StringUtils.equals(vo.getDicName(), dicName)) {
						return vo;
					}
				}
			}
	...
	public static boolean isAdminWithActivate() {
		List<RoleSimplifyDTO> datas = ShiroUtils.getAdministrator().getRoles();
		if (datas != null && datas.size() > 0) {
			for (RoleSimplifyDTO role : datas) {
				if (StringUtils.equals(role.getName(), "AdminWithActivate")) {
					return true;
				}
			}
		}
