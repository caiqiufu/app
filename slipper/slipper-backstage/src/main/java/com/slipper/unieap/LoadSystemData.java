...
public class LoadSystemData {
	...
	/**
	 * 加载应用基础配置参数,该参数在应用配置文件中application-dev.yml
	 */
	...
	/**
	 * 加载系统配置数据,在数据库中配置UNIEAP_SYS_CONFIG,在Handler前执行
	 */
	public void loadSystemConfigureData() {
		logger.info("====================================================================");
		logger.info("***            load unieap_sys_config data start                 ***");
		logger.info("====================================================================");
		...
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT NAME,VALUE FROM UNIEAP_SYS_CONFIG WHERE ACTIVATE_FLAG ='Y'");
		List<Map<String, Object>> datas = DBManager.getJT().queryForList(sql.toString());
		if (datas != null & !datas.isEmpty()) {
			String name, value = "";
			Map<String, Object> data;
		...
		logger.info("====================================================================");
		logger.info("***            load unieap_sys_config data end                   ***");
		logger.info("====================================================================");
	}
}
