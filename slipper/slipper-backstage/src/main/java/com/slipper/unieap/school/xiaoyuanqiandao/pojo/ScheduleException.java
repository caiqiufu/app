...
@Table(name = "school_schedule_exception")
public class ScheduleException implements Serializable {
	...
	@Column(nullable = false, length = 16, columnDefinition = "BIGINT(20) COMMENT 'work schedule ID'")
	private Long scheduleId;
	
	@Temporal(TemporalType.DATE)
	@Column(nullable = true, columnDefinition = "DATETIME COMMENT '特殊日期,必须打卡的日期'")
	private Date checkinDate;
	
	@Temporal(TemporalType.DATE)
	@Column(nullable = true, columnDefinition = "DATETIME COMMENT '特殊日期,不用打卡的日期'")
	private Date nocheckinStart;
	
	@Temporal(TemporalType.DATE)
	@Column(nullable = true, columnDefinition = "DATETIME COMMENT '特殊日期,不用打卡的日期'")
	private Date nocheckinEnd;
