...
@Table(name = "projectoa_project_attribute")
public class ProjectAttribute implements Serializable {
	...
	@Column(nullable = false, length = 20, columnDefinition = "BIGINT(20) COMMENT '属性ID'")
	private Long attributeId;
	
	@Column(nullable = true, length = 512, columnDefinition = "varchar(512) COMMENT '属性值'")
	private String attributeValue;

	@Column(nullable = false, length = 20, columnDefinition = "BIGINT(20) COMMENT '顺序'")
	private Long seq;

	@OneToOne(targetEntity = AttributeDefine.class)
	@JoinColumn(name = "id", referencedColumnName = "attributeId")
	private AttributeDefine attributeDefine;
