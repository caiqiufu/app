...
public class IGDSTask {
	...
	@Scheduled(cron = "0 0/10 6-23 * * 1") // 周一06:00-23:00 每间隔15分钟执行一次
	...
	@Scheduled(cron = "0 0/5 6-23 * * 1") // 周一06:00-23:00 每间隔15分钟执行一次
	// @Scheduled(cron="0 5 * * * *") //每小时第5分钟执行
	...
	@Scheduled(cron = "0 0/10 0-4,6-23 * * 2-5") // 周二到周五每间隔15分钟执行一次，05:00-06:00 不执行
	...
	@Scheduled(cron = "0 0/5 0-4,6-23 * * 2-5") // 周二到周五每间隔15分钟执行一次，05:00-06:00 不执行
	...
	@Scheduled(cron = "0 0/10 0-4 * * 6") // 周六每间隔15分钟执行一次，0-4点执行
	...
	@Scheduled(cron = "0 0/5 0-4 * * 6") // 周六每间隔15分钟执行一次，0-4点执行
