...
public class BaseBO {
	...
	public void setCriteria(DetachedCriteria criteria, Object bean) throws Exception {
		if (bean != null) {
			Map<String, PropertyDescriptor> beanprops = cacheBO.getBeanprops(bean);
	...
	public R getInfo(PaginationSupport page, String repositoryName, Object vo) throws Exception {
		UnieapRepository<?> repository = (UnieapRepository<?>) ApplicationContextProvider.getBean(repositoryName);
		repository.getPaginationDataByMysql(page, null, vo);
		if (page.items != null && page.items.size() > 0) {
			return R.success(page.items.get(0));
		} else {
			return R.success();
		}
	}
